...
- **Automatic Banner term discovery.** Planner and Admin schedule tools now read Banner's public term list, surface future planning terms without new static schedule files, and let admins refresh one discovered term or the next few future terms.
- **Cloud Scheduler live schedule readiness.** Live schedule refreshes now record manual vs scheduled run summaries, expose latest attempt/success details in Admin Dashboard, and include an ops runbook for creating paused Cloud Scheduler jobs after Banner reachability is verified.
- **Grade forgiveness/repeat guidance KB.** Added a dedicated procedure document for grade-forgiveness style questions that routes course repeats, exclusions, and GPA-record questions to Registrar Student Records while keeping grade appeals on the CS/SCMNS appeal path.
- **Degree profile foundation.** Curriculum and Ripple Effect can now resolve explicit CS, legacy Cloud Computing, and draft-review Artificial Intelligence degree profiles so advising/planner work can stop assuming every student follows the CS B.S. path.
- **AI degree course facts.** Added verified course records for AI-supporting MATH, MGBU, CLCO, and COSC requirements/electives supplied from catalog screenshots, reducing placeholder data in the draft AI profile.
- **What-If audit protection.** DegreeWorks What-If uploads now preserve existing completed/in-progress course history when a prior official record exists, while still letting Ripple Effect test another degree profile such as Artificial Intelligence.
- **AI live-seat refresh coverage.** Banner live schedule refresh now includes AI-supporting `MATH` and `MGBU` subjects by default alongside `COSC`, `CLCO`, `BIOI`, and GenEd subjects.
- **AI degree KB grounding.** Added source-backed structured KB documents for the Artificial Intelligence B.S. requirements, course prerequisites, Cloud Computing transition notes, What-If audit caveats, and live-seat advising guidance.
- **Profile major sync from DegreeWorks.** DegreeWorks PDF upload and Banner auto-sync now update the visible profile major from the synced degree program, so AI majors no longer keep the old default Computer Science label.
- **AI major profile selection.** Profile and legacy advising major dropdowns now include Artificial Intelligence, and the Profile dropdown uses exact degree-program values for CS, AI, and legacy Cloud Computing.
- **Admin local data visibility.** Admin Courses now reads the full local `classes.json` catalog, including AI course facts, and local KB file browsing includes `kb_structured` docs without touching the cloud datastore.
- **Advising beta feature gate.** Planner V2 and Scholarships are temporarily admin-only in routing and navigation while AI planner validation and scholarship readiness checks finish.
